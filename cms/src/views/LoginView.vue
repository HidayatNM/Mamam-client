...
export default {
  ...
  methods: {
    ...mapActions(useMamamStore, ['login']),
    loginMerchant() {
...
<template>
  <div class="container flex justify-center m-8 mx-auto">
    <div
      ...
      <form class="space-y-6" action="#" @submit.prevent="loginMerchant()">
        <h5 class="text-xl font-medium text-gray-900 dark:text-white">Sign in to our platform</h5>
        <div>
          <label for="email" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Your email</label>
          <input type="email" name="email" id="email"
            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white"
            placeholder="name@company.com" v-model="input.email" required>
        </div>
        <div>
        ...
        <div class="text-sm font-medium text-gray-500 dark:text-gray-300">
          Not registered?
          <router-link class="text-blue-700 hover:underline dark:text-blue-500" to="/register">Create
            account</router-link>
        </div>
      </form>
    </div>
